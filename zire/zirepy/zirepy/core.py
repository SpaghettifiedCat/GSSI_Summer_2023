...
def draw_zire_2(triggerCount, data, gain="LG"):
    #define Matplotlib figure and axis
    POST=f"_{gain}"
    event = data.iloc[triggerCount]
    ...
    MIN2 = np.min([event[CALOG[i+'6']+POST] + event[CALOG[i+'3']+POST] + event[CALOG[i+'1']+POST] for i in [str(i) for i in range(1, 9)]])
    MAX2 = np.max([event[CALOG[i+'6']+POST] + event[CALOG[i+'3']+POST] + event[CALOG[i+'1']+POST] for i in [str(i) for i in range(1, 9)]]) + 1

    ax.add_patch(Rectangle((160+10,     32.5), 7.5, 7.5,                edgecolor=edgec, lw=linewidth, facecolor=cm(int(  (event[CALOG['13']+POST]+event[CALOG['16']+POST] + event[CALOG['11']+POST] - MIN2)/(MAX2-MIN2)*256  ))))
    ax.add_patch(Rectangle((160+17.5,   32.5), 7.5, 7.5,                edgecolor=edgec, lw=linewidth, facecolor=cm(int(  (event[CALOG['23']+POST]+event[CALOG['26']+POST] + event[CALOG['21']+POST] - MIN2)/(MAX2-MIN2)*256  ))))
    ax.add_patch(Rectangle((160+10,     32.5-7.5), 7.5, 7.5,            edgecolor=edgec, lw=linewidth, facecolor=cm(int(  (event[CALOG['33']+POST]+event[CALOG['36']+POST] + event[CALOG['31']+POST] - MIN2)/(MAX2-MIN2)*256  ))))
    ax.add_patch(Rectangle((160+17.5,   32.5-7.5), 7.5, 7.5,            edgecolor=edgec, lw=linewidth, facecolor=cm(int(  (event[CALOG['43']+POST]+event[CALOG['46']+POST] + event[CALOG['41']+POST] - MIN2)/(MAX2-MIN2)*256  ))))
    ax.add_patch(Rectangle((160+10,     32.5-7.5-7.5), 7.5, 7.5,        edgecolor=edgec, lw=linewidth, facecolor=cm(int(  (event[CALOG['53']+POST]+event[CALOG['56']+POST] + event[CALOG['51']+POST] - MIN2)/(MAX2-MIN2)*256  ))))
    ax.add_patch(Rectangle((160+17.5,   32.5-7.5-7.5), 7.5, 7.5,        edgecolor=edgec, lw=linewidth, facecolor=cm(int(  (event[CALOG['63']+POST]+event[CALOG['66']+POST] + event[CALOG['61']+POST] - MIN2)/(MAX2-MIN2)*256  ))))
    ax.add_patch(Rectangle((160+10,     32.5-7.5-7.5-7.5), 7.5, 7.5,    edgecolor=edgec, lw=linewidth, facecolor=cm(int(  (event[CALOG['73']+POST]+event[CALOG['76']+POST] + event[CALOG['71']+POST] - MIN2)/(MAX2-MIN2)*256  ))))
    ax.add_patch(Rectangle((160+17.5,   32.5-7.5-7.5-7.5), 7.5, 7.5,    edgecolor=edgec, lw=linewidth, facecolor=cm(int(  (event[CALOG['83']+POST]+event[CALOG['86']+POST] + event[CALOG['81']+POST] - MIN2)/(MAX2-MIN2)*256  ))))

    gradient = np.linspace(0, 1, 100)
    gradient = np.vstack((gradient, gradient))
    plt.imshow(gradient, aspect='auto', cmap='viridis', extent=[50, 150, 1, 2])
    plt.imshow(gradient, aspect='auto', cmap='viridis', extent=[175-5, 190-5, 1, 2])



    plt.text(43, 3, f'{MIN}')
    plt.text(151, 3, f'{MAX}')

    plt.text(168-5, 3, f'{MIN2}')
    plt.text(191-5, 3, f'{MAX2}')
...
def draw_hist_calo(data, gain):
    bins = [i for i in range(5500, 12000, 20)]
    asics = [f'Asic{2}_CH{i}_{gain}' for i in range(2, 26)]
    data[asics].hist(figsize=(50, 50), bins=bins)

def draw_heatmap_2(data, gain):
    sum = data.sum().to_frame().transpose()
    draw_zire_2(0, sum, gain=gain)

...
def plot_ch(file_path, asic, ch, bin):
    ...
    # col 6 is first ch
    id = (asic*32+ch)*2+4
    df_hg = (pd.read_csv(file_path + ".csv", header=None, low_memory=False)[id][1:]).astype(int)
    df_lg = (pd.read_csv(file_path + ".csv", header=None, low_memory=False)[id+1][1:]).astype(int)
